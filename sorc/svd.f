      subroutine SVD(ndata,ma,x,y,XNEW,YNEW)
...
c     SUBROUTINE svdfit(x,y,sig,ndata,a,ma,u,v,w,mp,np,chisq,funcs)
      ...
      REAL chisq,a(ma),sig(ndata),u(ndata,np),v(np,np),w(np),x(ndata),
     *y(ndata),TOL
c     EXTERNAL funcs
      PARAMETER (TOL=1.e-5)
...
CC   sequentially wiwth different b's.
      ...
      INTEGER m,mp,n,np,NMAX
      REAL b(m),u(m,np),v(np,np),w(np),x(np)
!      PARAMETER (NMAX=MP)
      INTEGER i,j,jj
      REAL s,tmp(N)
      do 12 j=1,n
