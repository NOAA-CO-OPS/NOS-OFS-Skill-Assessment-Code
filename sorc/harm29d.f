C compile command: lf95 harm29.f utility.f -o harm29.x
C run command: harm29.x $KINDAT  $NCON $DELT $LONGITUDE $FILEIN > out
C
C  KINDAT:  =1 for vector data (current speed and direction)
C           =2 dor scalar (water level)
C    NCON:  number of constituents to be analyzed by H.A., maximum=37
C    DELT:  equally-spaced data time interval in minutes, e.g.6 for 6 minutes time interval
C LONGITUDE:longitude of the station in decimal degrees (positive for west longitude)
C  FILEIN:  Input data file name, data are in the following format (see input file,"W8638863_CB.obs"):
C   Julian_day YYYY MM DD HH MN value
C   Julian_day YYYY MM DD HH MN speed dir
C   
...
C     FOURIER - HARMONIC ANALYSIS    PROJECT 131403   29 DAYS
...
C              ACCELERATION IN EPOCH OF K(1) DUE TO P(1), 29 DAYS
      DATA(TAB1(JZ),JZ=1,18)/11.4,16.4,18.3,17.6,15.2,11.7,7.4,2.7,
     1 -2.2,-6.9,-11.3,-14.9,-17.5,-18.3,-16.7,-12.1,-4.7,3.9/ 
C              RATIO OF INCREASE IN AMPLITUDE OF K(1) DUE TO P(1),29DAYS
      DATA(TAB2(JZ),JZ=1,18)/-0.26,-0.17,-0.06,0.04,0.14,0.23,0.28,
     1 0.31,0.32,0.29,0.23,0.15,0.05,-0.06,-0.16,-0.25,-0.30,-0.31/
C              ACCELERATION IN EPOCH OF S(2) DUE TO K(2), 29DAYS
      DATA(TAB3(JZ),JZ=1,18)/5.9,9.6,12.6,14.6,15.0,13.5,9.6,3.7,
     1 -3.0,-9.1,-13.2,-15.0,-14.7,-12.9,-10.0,-6.4,-2.3,1.9/
C              RATIO OF INCREASE IN AMPLITUDE OF S(2) DUE TO K(2),29DAYS
      DATA(TAB4(JZ),JZ=1,18)/0.24,0.19,0.12,0.04,-0.05,-0.14,-0.21,
     1 2*-0.25,-0.21,-0.15,-0.06,0.03,0.11,0.18,0.23,2*0.26/
C              ACCELERATION IN EPOCH OF S(2) DUE TO T(2), 29 DAYS
      DATA(TAB5(JZ),JZ=1,37)/-0.8,-1.3,-1.8,-2.2,-2.6,-2.9,-3.2,
     1 3*-3.3,-3.1,-2.8,-2.5,-2.0,-1.5,-0.9,-0.3,0.3,0.9,1.5,2.0,2.4,
     2 2.8,3.1,3*3.3,3.2,3.0,2.7,2.3,1.9,1.4,0.8,0.3,-0.2,-0.8/
C              RESULTANT AMPLITUDE IN S(2) DUE TO T(2), 29 DAYS
      DATA(TAB6(JZ),JZ=1,37)/1.06,2*1.05,1.04,2*1.03,1.02,1.01,1.00,
     1 0.99,0.98,0.97,0.96,2*0.95,4*0.94,2*0.95,0.96,0.97,0.98,0.99,
     2 1.00,1.01,1.02,2*1.03,1.04,2*1.05,4*1.06/ 
C      SPEEDS OF CONSTITUENTS M(2),S(2),N(2),0(1),K(1)  FOR 1 SAMPLE/HR.
      DATA SXM2,SXS2,SXN2,SXO1,SXK1/0.1610228,0.1666667,0.1579985,
     1 0.0774613,0.0835615/
C              AMPLITUDE EFFECT OF CONSTITUENTS ON M(2), 29 DAYS
      DATA(TXXIXA( J ),J=1,20)/0.0,0.050,0.018,2*0.0,0.056,0.050,
     1 0.049,0.021,0.060,0.096,0.018,0.096,7*0.0/
C              AMPLITUDE EFFECT OF CONSTITUENTS ON N(2), 29 DAYS
      DATA(TXXIXB( J ),J=1,20)/0.050,0.00,0.005,2*0.0,0.052,0.049,
     1 0.050,0.031,0.021,0.018,0.096,0.968,7*0.0/
C              AMPLITUDE EFFECT OF CONSTITUENTS ON S(2), 29 DAYS
      DATA(TXXIXC( J ),J=1,20)/0.018,0.005,3*0.0,0.959,0.096,0.017,
     1 2*0.990,0.050,0.018,0.042,7*0.0/
C              AMPLITUDE EFFECT OF CONSTITUENTS ON 0(1), 29 DAYS
      DATA(TXXIXD( J ),J=1,20)/4*0.0,0.056,8*0.0,0.052,0.065,0.052,
     1 0.018,0.050,0.049,0.096/
C              AMPLITUDE EFFECT OF CONSTITUENTS ON K(1), 29 DAYS
      DATA(TXXIXE( J ),J=1,20)/3*0.0,0.056,9*0.0,2*0.050,0.056,0.959,
     1 0.052,0.049,0.011/
C              PHASE EFFECT OF CONSTITUENTS ON M(2), 29 DAYS
      DATA(TXXIXF( J ),J=1,20)/0.0,351.,174.,2*0.0,22.,9.,341.,8.,
     1 159.,164.,186.,196.,7*0.0/
C              PHASE EFFECT OF CONSTITUENTS ON N(2), 29 DAYS
      DATA(TXXIXG( J ),J=1,20)/9.,0.0,3.,2*0.0,32.,19.,351.,17.,
     1 169.,174.,196.,25.,7*0.0/
C              PHASE EFFECT OF CONSTITUENTS ON S(2),29 DAYS
      DATA(TXXIXH( J ),J=1,20)/186.,357.,3*0.0,29.,196.,348.,14.,
     1 346.,351.,193.,202.,7*0.0/
C              PHASE EFFECT OF CONSTITUENTS ON 0(1), 29 DAYS
      DATA(TXXIXI( J ),J=1,20)/4*0.0,22.,8*0.0,32.,13.,44.,174.,
     1 351.,341.,196./
C              PHASE EFFECT OF CONSTITUENTS ON K(1), 29 DAYS
      DATA(TXXIXJ( J ),J=1,20)/3*0.0,338.,9*0.0,9.,351.,22.,331.,
     1 328.,319.,354./ 
...
C     EQUILIBRIUM THEORY INFERENCE CONSTANTS
      DATA PK2SM,PL2MN,P2NMN,PR2SM,PT2SM,PLM2SM,PMU2SM,PNU2MN,PJ1KO,
     1 PM1KO,POO1KO,PP1KO,PQ1KO,P2Q1KO,PRH1KO/0.081,1.000,2.000,
     2 0.040,0.040,0.536,2.000,0.866,0.496,0.500,1.000,0.075,1.496,
     3 1.992,1.429/
      DATA RK2S2,RL2N2,R2NN2,RT2S2,RR2S2,RMU2M2,RLM2M2,RNU2N2,RJ1O1,
     1 RM1O1,ROO1O1,RP1K1,RQ1O1,R2Q1O1,RRH1O1/0.272,0.143,0.133,
     2 0.059,0.008,0.024,0.007,0.194,0.079,0.071,0.043,0.331,0.194,
     3 0.026,0.038/
      DATA(PX(I),I=1,24)/1.,1.,2.,2.,1.,2.,4.,6.,8.,2.,2.,1.,1.,1.,
     *                   1.,1.,2.,2.,4.,6.,2.,2.,2.,1./
    5 FORMAT( 28X,'1/F(K2)', 6X,'RA', 8X, 'QA', 6X, 'H~O(1)')
    6 FORMAT(1H ,25X, F9.4, 2F10.5, F10.4/)
    7 FORMAT(21X,'Equilibrium (V + U) and Elimination (F) Arguments'/)
    9 FORMAT(42X,'Analysis and Inference' /)
   11 FORMAT(3X,' Elimination of Component Effects' //15X,'R(A)',5X,
     1 'Zeta',5X, 'Kappa', 5X, 'H(A)')
   13 FORMAT(1H1,28X,'HARMONIC ANALYSIS,  29-DAYS')
   14 FORMAT(1H1,20X,'HARMONIC ANALYSIS, 15-DAYS', 2X,'N(2) INFERRED')
   15 FORMAT(/ 14X,'Harmonic Constants',1X,'(H) and Kappa',A6/)
...
! 2039 FORMAT(4A8)
 ...
 4440 FORMAT(1H0, 10F12.4 /// 1X, 10F12.4)
 ...
 8008 TXXIX2(5,MAXJ) = TXXIXJ(MAXJ)
       lout=6
       PRINT*, '       ********** 29 DAY HARMONIC ANALYSIS**********'
       print*, ' '
       read(25,4701) tapein
 4701 format(a80)
!       call ncrght(tapein,ncut)
!       tapein=tapein(1:ncut)
      print *, '   analyze data in ',tapein
c      print*, ' '
c      print*, ' '
      DO 6005 I=1,16
      IDENS(I)='          '
...
!       IDENS(3) = ' Data in  '
       IDENS(3) = TAPEIN(1:10)
       IDENS(4) = TAPEIN(11:20)
       IDENS(5) = TAPEIN(21:30)
       IDENS(6) = TAPEIN(31:40)
      write(idens(7),271) RATIO
  271 format(3H R=,F6.3,X)
...
c
      ...
      IF(NSPH.EQ.0) STOP
C
C     USING REFERENCE STATION
C
      IF(IREF.LE.0) GO TO 30
      READ 31,(H(I),PK(I),I=1,37)
      PRINT 311,(H(I),PK(I),I=1,37)
   31 FORMAT(6(8X,7(F5.3,F4.1)/))
  311 FORMAT(6(8X,7(F6.3,x,F5.1,x)/))
C
      IF(H(1).LE.0.0) GO TO 32
      IF(H(13).GT.0.0) RMU2M2 = H(13)/H(1)
      IF(H(16).GT.0.0) RLM2M2 = H(16)/H(1)
   32 IF(H(2).LE.0.0) GO TO 33
      IF(H(35).GT.0.0) RK2S2 = H(35)/H(2)
      IF(H(27).GT.0.0) RT2S2 = H(27)/H(2)
      IF(H(28).GT.0.0) RR2S2 = H(28)/H(2)
   33 IF(H(3).LE.0.0) GO TO 36
      IF(H(33).GT.0.0) RL2N2 = H(33)/H(3)
      IF(H(14).GT.0.0) R2NN2 = H(14)/H(3)
      IF(H(11).GT.0.0) RNU2N2 = H(11)/H(3)
   36 IF(H(30).GT.0.0.AND.H(4).GT.0.0) RP1K1 = H(30)/H(4)
      IF(H(6).LE.0.0) GO TO 37
      IF(H(19).GT.0.0) RJ1O1 = H(19)/H(6)
      IF(H(18).GT.0.0) RM1O1 = H(18)/H(6)
      IF(H(15).GT.0.0) ROO1O1 = H(15)/H(6)
      IF(H(26).GT.0.0) RQ1O1 = H(26)/H(6)
      IF(H(29).GT.0.0) R2Q1O1 = H(29)/H(6)
      IF(H(25).GT.0.0) RRH1O1 = H(25)/H(6)
   37 IF(H(1).LE.0.0.OR.H(2).LE.0.0) GO TO 38
      IF(H(35).GT.0.0) PK2SM = (PK(35)-PK(1))/(PK(2)-PK(1))
      IF(H(27).GT.0.0) PT2SM = (PK(27)-PK(1))/(PK(2)-PK(1))
      IF(H(28).GT.0.0) PR2SM = (PK(28)-PK(1))/(PK(2)-PK(1))
      IF(H(16).GT.0.0) PLM2SM= (PK(16)-PK(1))/(PK(2)-PK(1))
      IF(H(13).GT.0.0) PMU2SM= (PK(13)-PK(1))/(PK(2)-PK(1))
   38 IF(H(1).LE.0.0.OR.H(3).LE.0.0) GO TO 39
      IF(H(33).GT.0.0) PL2MN = (PK(33)-PK(3))/(PK(1)-PK(3))
      IF(H(14).GT.0.0) P2NMN = (PK(14)-PK(3))/(PK(1)-PK(3))
      IF(H(11).GT.0.0) PNU2MN= (PK(11)-PK(3))/(PK(1)-PK(3))
   39 IF(H(4).LE.0.0.OR.H(6).LE.0.0) GO TO 30
      IF(H(19).GT.0.0) PJ1KO = (PK(19)-PK(6))/(PK(4)-PK(6))
      IF(H(18).GT.0.0) PM1KO = (PK(18)-PK(6))/(PK(4)-PK(6))
      IF(H(15).GT.0.0) POO1KO= (PK(15)-PK(6))/(PK(4)-PK(6))
      IF(H(30).GT.0.0) PP1KO = (PK(30)-PK(6))/(PK(4)-PK(6))
      IF(H(26).GT.0.0) PQ1KO = (PK(26)-PK(6))/(PK(4)-PK(6))
      IF(H(29).GT.0.0) P2Q1KO= (PK(29)-PK(6))/(PK(4)-PK(6))
      IF(H(25).GT.0.0) PRH1KO= (PK(25)-PK(6))/(PK(4)-PK(6))
C
   30 DO 1111 JN = 1,NJ
      JIT = IIT
      JOBX=29
...
c
      IF(IAND.LE.0) GO TO 6200
       open(11,file=trim(tapein),status='old',form='unformatted',
     #access='sequential')
       print*, ' '
...
!      READ (9,AQ,END=1010)VIN(I)
 ...
 1010 NE = I-1
      print *, 'NE= ',NE
      PRINT 3042,VIN(1),0.0,VIN(NE),0.
  123 NECOM = 1
  208 PRINT 3043,ISKIP9+1,VIN(ISKIP9+1)
      PRINT 3043,N29+ISKIP9,VIN(N29+ISKIP9)
      IF(N29+ISKIP9.gt.NE) then
       print*, '    ATTENTION '
       print*, '    ATTEMPTED TO SKIP TOO MANY POINTS'
       print*, ' '
       print*, '   last point is # ',ne,'   needed  ',n29+iskip9
       go to 1111
       endif
      N = N29
      NSAVE = NE
...
C
      CALL XMEAN(XTEMP,SUTZ,N29,AVE,2)
C
      GO TO 108
  102 DO I=1,N
        VIN(I)=RDATA(I,1)
...
!      IF (ID.NE.IDD) PRINT 3038,IN2
 ...
 3009 PRINT 3043,ISKIP9+1,VIN(ISKIP9+1),DIN(ISKIP9+1)
      PRINT 3043,ISKIP9+N29,VIN(ISKIP9+N29),DIN(ISKIP9+N29)
       if(iskip9+n29.gt.ne) then
       print*, '    ATTENTION '
       print*, '    ATTEMPTED TO SKIP TOO MANY POINTS'
       print*, ' '
       print*, '  last point is ',ne,'   needed  ',iskip9+n29
       go to 1111
       endif
C
...
C     (WITH INFERENCE)
 ...
 8059 DO 21 I = 1,5
   21 CKAPAP(I) = CHSIP(I) + COVU(I)
      ...
      SMD = CKAP(3) - CKAP(1)
      CALL ONEPI(SMD)
      CKAP(6) = CKAP(3) + PK2SM*SMD
      DMN = CKAP(1)- CKAP(2)
      CALL ONEPI( DMN)
      CKAP(7) = CKAP(1) + PL2MN*DMN
      CKAP(8) = CKAP(1) - P2NMN*DMN
      CKAP(9) = CKAP(3) + PR2SM*SMD
      CKAP(10)= CKAP(3) - PT2SM*SMD
      CKAP(11)= CKAP(3) - PLM2SM*SMD
      CKAP(12)= CKAP(3) - PMU2SM*SMD
      CKAP(13)= CKAP(1) - PNU2MN*DMN
      OKD = CKAP(5) - CKAP(4)
      CALL ONEPI(OKD)
      CKAP(14)= CKAP(5) + PJ1KO*OKD
      CKAP(15)= CKAP(5) - PM1KO*OKD
      CKAP(16)= CKAP(5) + POO1KO*OKD
      CKAP(17)= CKAP(5) - PP1KO*OKD
      CKAP(18)= CKAP(5) - PQ1KO*OKD
      CKAP(19)= CKAP(5) - P2Q1KO*OKD
      CKAP(20)= CKAP(5) - PRH1KO*OKD
      CALL TWOPI(CKAP,20)
C     DETERMINATION OF ZETA(A) AND R(A) FOLLOWS
C     (WITH INFERENCE)
      DO 22 I = 1,20
   22 CHSI(I) = CKAP(I) - COVU(I)
      CALL TWOPI(CHSI, 20)
      CR(1) = CRP(1)
      CR(2) = CRP(2)
      CR(3) = CRP(3)/PROD
      ...
      CR(12) = RMU2M2*CR(1)
      CR(13) = RNU2N2*CR(2)
      CR(14) = RJ1O1*AHNAT/CF(14)
      CR(15) = RM1O1*CR(4)/CQA
      CR(16) = ROO1O1*AHNAT/CF(16)
      CR(17) = RP1K1*CR(5)*CF(5)
      CR(18) = RQ1O1*CR(4)
      CR(19) = R2Q1O1*CR(4)
      CR(20) = RRH1O1*CR(4)
C     ELIMINATION OF COMPONENT EFFECTS AND DETERMINE KAPPA AND H(A)
   40 DO 35 I=1,5
      ...
      SOQ = 0.0
      DO 20 J = 1,20
  331 BELL(J) = CR(J)*TXXIX1(I,J)
      IF(BELL(J)-.0005)23,24,24
   23 BELL(J) = 0.0
   24 BINGO(J)= TXXIX2(I,J) - CHSI(J) + CHSIP(I)
      IF(BINGO(J))25,28,26
   25 BINGO(J) = ( BINGO(J) + 360.)*0.0174533
      GO TO 29
   26 IF(BINGO(J) - 360.0) 28,28,27
   27 BINGO(J) = (BINGO(J) + 360.0)*0.0174533
      GO TO 29
   28 BINGO(J) = BINGO(J)*0.0174533
   29 SUM = SUM + BELL(J)*SIN(BINGO(J))
   20 SOQ = SOQ + BELL(J)*COS(BINGO(J))
      QOS = CRP(I) - SOQ
      CALL FITAN(SUM,QOS,DCCH,2)
      CDCH = DCCH*0.0174533
      CRC(I) = 0.0
      IF(DCCH.EQ.90.0.OR.DCCH.EQ.270.0) GO TO 34
      CRC(I) = (CRP(I) - SOQ)/ COS(CDCH)
   34 G(I) = CRC(I)*CF(I)
      ZETA(I) = CHSIP(I) + CDCH*57.29578
   35 PAK(I) = CHSIP(I) + CDCH*57.29578 + COVU(I)
      CALL TWOPI(ZETA,5)
      CALL TWOPI(PAK, 5)
      D(1) = RJ1O1*G(4)
      D(2) = G(5)
...
C     INFERENCE  OF   KAPPA(A)  FROM MAJOR CONSTITUENTS
 ...
 6781 CALL ONEPI(PATO)
      CALL ONEPI(PATTO)
      W(1) = PAK(5) + PJ1KO*PAFX
      W(2) = PAK(5)
      W(3) = PAK(3) + PK2SM*PATTO
      W(4) = PAK(1) - PL2MN*PAOT
      W(5) = PAK(5) - PM1KO*PAFX
      W(6) = PAK(1)
      W(7) = CHSIP(6) + COVU(21)
      W(8) = CHSIP(7) + COVU(22)
      W(9) = CHSIP(8) + COVU(23)
      W(10) = PAK(2)
      W(11)= PAK(1) - P2NMN*PAOT
      W(12) = PAK(4)
      W(13)= PAK(5) + POO1KO*PAFX
      W(14)= PAK(5) - PP1KO*PAFX
      W(15)= PAK(5) - PQ1KO*PAFX
      W(16)= PAK(5) - P2Q1KO*PAFX
      W(17)= PAK(3) + PR2SM*PATTO
      W(18) = PAK(3)
      W(19) = CHSIP(9) + COVU(24)
      W(20) = CHSIP(10) + COVU(25)
      W(21)= PAK(3) - PT2SM*PATTO
      W(22)= PAK(3) - PLM2SM*PATTO
      W(23)= PAK(1) - PNU2MN*PAOT
      W(24)= PAK(5) - PRH1KO*PAFX
      CALL TWOPI(W,24)
C
C     GETTING READY FOR ANOTHER ELIMINATION
C
      JIT = JIT - 1
      IF(JIT.LE.0) GO TO 70
      CKAP(1) = W(6)
      CKAP(2) = W(10)
      CKAP(3) = W(18)
      CKAP(4) = W(12)
...
C
c     CALL XMEAN(XTEMP,SUTZ,N29,AVEDM,1)
C
      IF(NECOM.LT.3.OR.NECOM.GT.4) GO TO 809
      IF(K.EQ.1) GO TO 809
!      CALL VELDIR1(ASAV,AVE,VL,DR,AZI)
...
C     INTERPOLATE TABLES 21,22,23,24,25,AND 26
    ...
    1 FORMAT(///22H JOB TERMINATED.......,26HFAILED TO INTERPOLATE ANG.)
      LIN = LID - 1
      DO 200  ION = 1,LIN
...
C
  ...
  400 CALL NTRAN (N,2,34,IHEAD,IS,22)
   ...
   21 FORMAT(' SUMMARY OF HEADING INFORMATION FOR THIS FILE',//, 
     ...
     2 A4,A4,A2,//,' METER SERIAL NUMBER ',I7,/,
     3 ' METER REFERENCE NUMBER ',I4,//,' LATITUDE,  DIRECTION = ', 
     4 A1,', DEGREES = ',I3,', MINUTES = ',I2, ', SECONDS = ',I2,/,
     5 ' LONGITUDE, DIRECTION = ',A1,', DEGREES = ',I3,
     6 ', MINUTES = ',I2,', SECONDS = ',I2,//,
     7 ' START TIME, MINUTES = ',I2, ', HOURS = ',I2,
     8 ', DAYS = ',I3,', YEAR  = ', I2,/,
     9 ' STOP  TIME, MINUTES = ',I2, ', HOURS = ',I2, 
     1 ', DAYS = ',I3,', YEAR  = ', I2,//,  
     2 ' TIME ANCHOR DROPPED, MINUTES =',I3,', HOURS =',I3,
     3 ', DAYS = ',I4,', YEARS = ',I3,/,
     4 ' TIME ANCHOR RELEASE, MINUTES =',I3,', HOURS =',I3,
     5 ', DAYS =',I4,', YEARS =',I3,//,' DEPTH OF METER =',I5,
     6 ' FEET,IF NEGATIVE, FEET FROM THE SURFACE',/,29X,
     7 'IF POSITIVE, FEET FROM THE BOTTOM'//) 
...
C
    ...
    5 FORMAT(1H0,13X,'AMPLITUDE',37X,'EPOCH' / 14X,'RELATIONS',35X,
     1 'RELATIONS' ///7X,'M(4)/M(2)',7X,F7.3,20X,'2M(2)-M(4)',   
     2 5X,F7.2 //7X,'M(6)/M(2)',7X,F7.3,20X,'3M(2)-M(6)',5X,F7.2 //  
     3 7X,'S(2)/M(2)',7X,F7.3,20X,'S(2)-M(2)',6X,F7.2 //  
     4 7X,'N(2)/M(2)',7X,F7.3,20X,'M(2)-N(2)',6X,F7.2 //  
     5 7X,'O(1)/K(1)',7X,F7.3,20X,'K(1)-O(1)',6X,F7.2 //
     6 7X,'K(1)+O(1)',7X,F7.3,20X,'K(1)+O(1)',6X,F7.2 //
     6 6X,'(K(1)+O(1))/M(2) ',F7.3,20X,'M(2)-K(1)-O(1) ',F7.2 //  
     7 50X,'MKO',12X,F7.2)                                          
    ...
    7 FORMAT(    22X,18HMEAN TIDE LEVEL = , F6.3 // )
    8 FORMAT(    15X,34HACC. IN HW DUE TO M(4) AND M(6)  =,F8.3 //  15X,
     134HACC. IN LW DUE TO M(4) AND M(6)  =, F8.3 /)                           
    9 FORMAT( 18X, 4F8.3)
   10 FORMAT(//   4X,15HGREENWICH HWI  , F7.3,15X,13HHW AMPLITUDE , F8.3
     1 //   4X,15HGREENWICH LWI   , F7.3,15X,13HLW AMPLITUDE , F8.3)
   11 FORMAT(//   5X,14HSPRING RANGE  , F8.3,11X,16HPERIGEAN RANGE  , F8      
     1.3  //  6X,13HNEAP  RANGE  , F8.3,11X,16HAPOGEAN  RANGE  , F8.3  )      
   12 FORMAT( // 27X,'TROPIC INTERVALS' // 11X,'TCHHWI  ',F7.3,' (A)',
     1 14X,'TCLLWI  ',F7.3,' (A)'  // 11X, 'TCLHWI  ', F7.3, ' (A)',     
     2 14X,'TCHLWI  ',F7.3,' (A)' )
   23 FORMAT(/// 12X,16HSEQUENCE OF TIDE)
   14 FORMAT(1H+,29X,29H    ( HHW    LW   LHW    LW ))
   15 FORMAT(1H+,29X,29H    ( HHW   LLW   LHW   HLW ))
   16 FORMAT(1H+,29X,29H    (  HW   LLW    HW   HLW ))
   17 FORMAT(1H+,29X,29H    ( LHW   LLW   HHW   HLW ))
   18 FORMAT(1H+,29X,29H    ( LHW    LW   HHW    LW ))
   19 FORMAT(1H+,29X,29H    ( LHW   HLW   HHW   LLW ))
   20 FORMAT(1H+,29X,29H    (  HW   HLW    HW   LLW ))
   21 FORMAT(1H+,29X,29H    ( HHW   HLW   LHW   LLW ))
   22 FORMAT(/// 26X,17HTROPIC    HEIGHTS //  12X, 7HTCHHW  , F8.3,18X,       
     17HTCLLW  , F8.3 // 12X, 7HTCLHW  , F8.3,18X, 7HTCHLW  , F8.3 )          
   24 FORMAT(/// 28X,14HMEAN   HEIGHTS //  13X, 6HMHHW  , F8.3,20X, 6HML 
     1LW  , F8.3 //  13X, 6HMLHW  , F8.3,20X, 6HMHLW  , F8.3 )           
   25 FORMAT(//  14X, 5HDHQ  , F7.3,21X, 5HDLQ  , F7.3 )
   26 FORMAT(/// 28X,14HDIURNAL   TIDE  // 11X, 8HTCHHWI  , F8.3,18X, 8H 
     1TCLLWI  , F8.3  // 12X, 7HTCHHW  , F8.3,19X, 7HTCLLW  , F8.3  // 1 
     23X, 6HMHHW  , F8.3,19X, 6HMLLW  , F8.3 )
   29 FORMAT(/// ' THE COMPOUND WAVE IS SIX-DIURNAL FOR ALL PHASE REL.')
   30 FORMAT(/// ' DUE TO M(6), DOUBLE AMPLITUDES HAVE OCCURED ' // ' SE
     1E MANUAL OF HARMONIC CONSTANT REDUCTION  ( PAGES 38-39 ) ' )
   31 FORMAT( /// ' COMPOUND WAVE IS QUARTER-DIURNAL FOR ALL PHASE RELAT
     1IONS ' )
   32 FORMAT(/// ' A DOUBLE LOW AMPLITUDE IS PRESENT ' )
   33 FORMAT(/// ' A DOUBLE HIGH AMPLITUDE IS PRESENT ' )
   34 FORMAT(/// ' DUE TO M(4), THE WAVE IS APPROACHING CRITICAL LIMITS
     1 FOR DOUBLE AMPLITUDES TO OCCUR ' )
   37 FORMAT(/// ' DOUBLE HIGH AMPLITUDES OF EQUAL HEIGHTS AND DOUBLE LO
     1W AMPLITUDES OF EQUAL HEIGHTS ARE PRESENT ' )
   38 FORMAT(/// ' DUE TO M(6), THE COMPOUND WAVE IS SIX-DIURNAL FOR ALL
     1 PHASE RELATIONS ' )
   39 FORMAT(// 10F10.4)
   40 FORMAT( 12X, 4F10.4)
   41 FORMAT(12X, 2F10.4)
   42 FORMAT(/// 22X,12HMEAN RANGE   , F8.4)
   43 FORMAT(////// 1H0,38H**** COMPUTE MEAN HEIGHTS MANUALLY ***,
     146HMEAN HEIGHT SUBROUTINE HAS TO BE DEBUGGED ****)
...
C         TABLE 16
  ...
  888 RETURN
      ...
      TLL = 2.0*HP1
      TVAL = 0.386*TAA
      IF(TVAL.GT.TLL) GO TO 9
      IF(TLL.EQ.0.0) GO TO 9
      XL = TAA/TLL
