...
C   Day_end is the end time of the new series in days.
      ...
      SUBROUTINE EQUAL_INTERVAL1(DAY_BEGIN,DAY_END,DELTA,DELT0,METHOD,
     1  CRITERIA1,CRITERIA2,NUMT,TIME,WL,TIME_NEW,WL_NEW,NUM,M_NEW)

      PARAMETER(NDATA = 550000,MA = 5)
      REAL*8 DELTA,DELT0,TIME,TIME_NEW(NUMT),DAY_BEGIN,DAY_END
      REAL*8 HOUR_BEGIN,HOUR_END,TIME0,T1,GAP,TIMEB,TIMEE
...
10      CONTINUE
        ...
        IF((J0 .GT. 1) .AND. (TIME(J0) .LT. TIME(1)+TWINDOW)) THEN
          IF(GAP .LE. CRITERIA1 ) THEN
            TIMEB = TIME(J0-1)
            TIMEE = TIME(J0)
            YB = WL(J0-1)
            YE = WL(J0)
            CALL LINEAR_DP(TIMEB,YB,TIMEE,YE,T1,Y0)
            TIME_NEW(I) = T1
            WL_NEW(I) = Y0
          ELSE
            TIME_NEW(I) = T1
     ...
     1         (TIME(N).LE. TIME(J0)+TWINDOW/2)) THEN
          ...
          ELSE IF(NFIT .GE. NWIN/2) THEN 
            IF(GAP .LE. CRITERIA1) THEN
              ...
              TIME_NEW(I) = T1
              WL_NEW(I) = Y0
            ELSE IF((GAP.GT.CRITERIA1).AND.(GAP.LE.CRITERIA2)) THEN
              IF(METHOD .EQ. 0) THEN
                CALL SPLINE(NFIT,X,Y,real(T1),YNEW)
              ELSE IF(METHOD .EQ. 1) THEN
                CALL SVD(NFIT,MA,X,Y,real(T1),YNEW)
              ENDIF
              TIME_NEW(I) = T1
              WL_NEW(I) = YNEW
