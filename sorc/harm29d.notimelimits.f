...
C              ACCELERATION IN EPOCH OF K(1) DUE TO P(1), 29 DAYS
      DATA(TAB1(JZ),JZ=1,18)/11.4,16.4,18.3,17.6,15.2,11.7,7.4,2.7,-2.2, 
     1-6.9,-11.3,-14.9,-17.5,-18.3,-16.7,-12.1,-4.7,3.9/            
C              RATIO OF INCREASE IN AMPLITUDE OF K(1) DUE TO P(1),29DAYS
      DATA(TAB2(JZ),JZ=1,18)/-0.26,-0.17,-0.06,0.04,0.14,0.23,0.28,0.31,
     10.32,0.29,0.23,0.15,0.05,-0.06,-0.16,-0.25,-0.30,-0.31/ 
C              ACCELERATION IN EPOCH OF S(2) DUE TO K(2), 29DAYS
      DATA(TAB3(JZ),JZ=1,18)/5.9,9.6,12.6,14.6,15.0,13.5,9.6,3.7,-3.0,-9 
     1.1,-13.2,-15.0,-14.7,-12.9,-10.0,-6.4,-2.3,1.9/  
C              RATIO OF INCREASE IN AMPLITUDE OF S(2) DUE TO K(2),29DAYS
      DATA(TAB4(JZ),JZ=1,18)/0.24,0.19,0.12,0.04,-0.05,-0.14,-0.21,2*-0.
     125,-0.21,-0.15,-0.06,0.03,0.11,0.18,0.23,2*0.26/
C              ACCELERATION IN EPOCH OF S(2) DUE TO T(2), 29 DAYS
      DATA(TAB5(JZ),JZ=1,37)/-0.8,-1.3,-1.8,-2.2,-2.6,-2.9,-3.2,3*-3.3,-
     13.1,-2.8,-2.5,-2.0,-1.5,-0.9,-0.3,0.3,0.9,1.5,2.0,2.4,2.8,3.1,3*3.
     23,3.2,3.0,2.7,2.3,1.9,1.4,0.8,0.3,-0.2,-0.8/ 
C              RESULTANT AMPLITUDE IN S(2) DUE TO T(2), 29 DAYS
      DATA(TAB6(JZ),JZ=1,37)/1.06,2*1.05,1.04,2*1.03,1.02,1.01,1.00,0.99
     1,0.98,0.97,0.96,2*0.95,4*0.94,2*0.95,0.96,0.97,0.98,0.99,1.00,1.01
     2,1.02,2*1.03,1.04,2*1.05,4*1.06/
C      SPEEDS OF CONSTITUENTS M(2),S(2),N(2),0(1),K(1)  FOR 1 SAMPLE/HR.
      DATA SXM2,SXS2,SXN2,SXO1,SXK1/0.1610228,0.1666667,0.1579985,0.0774
     1613,0.0835615/
C              AMPLITUDE EFFECT OF CONSTITUENTS ON M(2), 29 DAYS
      DATA(TXXIXA( J ),J=1,20)/0.0,0.050,0.018,2*0.0,0.056,0.050,0.049,0
     1.021,0.060,0.096,0.018,0.096,7*0.0/
C              AMPLITUDE EFFECT OF CONSTITUENTS ON N(2), 29 DAYS
      DATA(TXXIXB( J ),J=1,20)/0.050,0.00,0.005,2*0.0,0.052,0.049,0.050,
     10.031,0.021,0.018,0.096,0.968,7*0.0/ 
C              AMPLITUDE EFFECT OF CONSTITUENTS ON S(2), 29 DAYS
      DATA(TXXIXC( J ),J=1,20)/0.018,0.005,3*0.0,0.959,0.096,0.017,2*0.9
     190,0.050,0.018,0.042,7*0.0/ 
C              AMPLITUDE EFFECT OF CONSTITUENTS ON 0(1), 29 DAYS
      DATA(TXXIXD( J ),J=1,20)/4*0.0,0.056,8*0.0,0.052,0.065,0.052,0.018
     1,0.050,0.049,0.096/
C              AMPLITUDE EFFECT OF CONSTITUENTS ON K(1), 29 DAYS
      DATA(TXXIXE( J ),J=1,20)/3*0.0,0.056,9*0.0,2*0.050,0.056,0.959,0.0
     152,0.049,0.011/ 
C              PHASE EFFECT OF CONSTITUENTS ON M(2), 29 DAYS
      DATA(TXXIXF( J ),J=1,20)/0.0,351.,174.,2*0.0,22.,9.,341.,8.,159.,1
     164.,186.,196.,7*0.0/ 
C              PHASE EFFECT OF CONSTITUENTS ON N(2), 29 DAYS
      DATA(TXXIXG( J ),J=1,20)/9.,0.0,3.,2*0.0,32.,19.,351.,17.,169.,174 
     1.,196.,25.,7*0.0/ 
C              PHASE EFFECT OF CONSTITUENTS ON S(2),29 DAYS
      DATA(TXXIXH( J ),J=1,20)/186.,357.,3*0.0,29.,196.,348.,14.,346.,35
     11.,193.,202.,7*0.0/ 
C              PHASE EFFECT OF CONSTITUENTS ON 0(1), 29 DAYS
      DATA(TXXIXI( J ),J=1,20)/4*0.0,22.,8*0.0,32.,13.,44.,174.,351.,341
     1.,196./ 
C              PHASE EFFECT OF CONSTITUENTS ON K(1), 29 DAYS
      DATA(TXXIXJ( J ),J=1,20)/3*0.0,338.,9*0.0,9.,351.,22.,331.,328.,31
     19.,354./ 
      DATA(ISP(I),I = 1,24)/155854433,150410686,300821373,295284789,1449
     166939,289841042,579682084,869523127,1159364169,284397295,278953548
     2,139430356,161391017,149589314,133986609,128542862,300410667,30000
     30000,600000000,900000000,299589333,294556253,285125831,134715145/
C     EQUILIBRIUM THEORY INFERENCE CONSTANTS
      DATA PK2SM,PL2MN,P2NMN,PR2SM,PT2SM,PLM2SM,PMU2SM,PNU2MN,PJ1KO,PM1K
     1O,POO1KO,PP1KO,PQ1KO,P2Q1KO,PRH1KO/0.081,1.000,2.000,0.040,0.040,
     20.536,2.000,0.866,0.496,0.500,1.000,0.075,1.496,1.992,1.429/
      DATA RK2S2,RL2N2,R2NN2,RT2S2,RR2S2,RMU2M2,RLM2M2,RNU2N2,RJ1O1,RM1O
     11,ROO1O1,RP1K1,RQ1O1,R2Q1O1,RRH1O1/0.272,0.143,0.133,0.059,0.008,0
     2.024,0.007,0.194,0.079,0.071,0.043,0.331,0.194,0.026,0.038/
      DATA(PX(I),I=1,24)/1.,1.,2.,2.,1.,2.,4.,6.,8.,2.,2.,1.,1.,1.,1.,1.
     *                  ,2.,2.,4.,6.,2.,2.,2.,1./
   ...
   13 FORMAT(1H1,28X,27HHARMONIC ANALYSIS,  29-DAYS )
...
c      end if
!       altout = 'cons.out'
       open(unit=10,file='cons.out',form=
...
C
    ...
    5 FORMAT(1H0,13X,'AMPLITUDE',37X,'EPOCH' / 14X,'RELATIONS',35X,'RELA
     1TIONS' ///  7X,'M(4)/M(2)', 7X, F7.3,20X,'2M(2)-M(4)', 5X, F7.2  /
     2/ 7X,'M(6)/M(2)', 7X, F7.3,20X,'3M(2)-M(6)', 5X,F7.2 //  7X,'S(2)/
     3M(2)', 7X, F7.3,20X,'S(2)-M(2)', 6X,F7.2 //  7X,'N(2)/M(2)', 7X, F
     47.3,20X,'M(2)-N(2)', 6X, F7.2 //  7X,'O(1)/K(1)', 7X, F7.3,20X,'K(
     51)-O(1)', 6X, F7.2 //  7X,'K(1)+O(1)', 7X, F7.3,20X,'K(1)+O(1)', 6
     6X, F7.2 //  6X,'(K(1)+O(1))/M(2) ', F7.3,20X,'M(2)-K(1)-O(1) ', F7
     7.2 // 50X, 'MKO',12X,F7.2)
   ...
   12 FORMAT( // 27X,'TROPIC INTERVALS' // 11X,'TCHHWI',F7.3,        4H
     1(A),14X, 8HTCLLWI  , F7.3, 4H (A)  //  11X, 8HTCLHWI  , F7.3, 4H (
     2A) ,14X, 8HTCHLWI  , F7.3, 4H (A) )
