...
#source STEPS_SETUP.sh
  ...
  cd $WRK_DIR
  echo get_WL_realtime.sh $BEGINDATE " to " $ENDDATE
  NCOL=5
  DATUM=0.0
...
#  Convert datum from IGLD 1985 to Chart Datum Low Water Datum  
  if [ $DBASE = "GLAKES" ]; then
    ...
    if [ $DELT_O = 60 -o $DELT_O = 60.0 ]; then
      echo "No hourly real time water level data"
      exit 
    elif [ $DELT_O = 6 -o $DELT_O = 6.0 ];  then
      TEMPLATE=$CTL/request.template_wl_realtime_6min_GL
    fi
  else
    if [ $DELT_O = 60 -o $DELT_O = 60.0 ]; then
      echo "No hourly real time water level data"
      ...
      TEMPLATE=$CTL/request.template_wl_realtime_6min
    fi
  fi
...
#  End of modification by Zheng
     ...
     if [ -s $WGETOUT ]; then
       awk -v sid=$stnid -v NCL=$NCOL -v datum=$DATUM ' $1 == sid && NF >= NCL \
          ...
          substr($5,1,2)" "substr($5,4,2)" "$6-datum }'  $WGETOUT > tmp.out
       cp tmp.out $WGETOUT 
     fi
  ...
  echo "done of get_WL_realtime_COOPS.sh!!!"
exit
